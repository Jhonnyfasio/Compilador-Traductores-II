...
void AnalizadorLexico::analizeSintaticString() {
    Lexico lexico;
...
/*void AnalizadorLexico::analizeToken(const std::string& aux, const int& auxInt) {
    if(aux == "IF"){
        analizeIF(aux, 0);
    }
    else if(aux == "WHILE"){
        analizeWHILE(aux, auxInt);
    }
    else if(aux == "FOR"){
        analizeFOR(aux);
    }
